...
In this part we use support vector machines models for classification and regression analysis.

```{r}
#creating a smaller dataset for fist tests
set.seed(143)
df_sample_small<-df[sample(nrow(df), 5000), ]
```

```{r}
df_sample_small%>%
  ggplot(aes(x = kilometer, y = price, color = notRepairedDamage)) +
  geom_jitter(alpha = 1)
```
```{r}
# Comparing the average price per brand repaired and unrepaired
svm_df_mbr<-df%>%
group_by(notRepairedDamage, brand) %>%
summarize(Mean = mean(price, na.rm=TRUE))%>%
arrange(brand)


svm_df_mbr%>%
  ggplot(aes(y = brand, x = Mean, color = notRepairedDamage)) +
  geom_point(alpha = 1)
```
...
# Comparing the average price per brand repaired and unrepaired
svm_df_mmr<-df%>%
group_by(notRepairedDamage, model) %>%
summarize(Mean = mean(price, na.rm=TRUE))%>%
arrange(model)


svm_df_mmr%>%
  ggplot(aes(x = model, y = Mean, color = notRepairedDamage)) +
  geom_point(alpha = 1)
```
