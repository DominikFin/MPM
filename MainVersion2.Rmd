...
plot(models)
```
The above plot shows the resulting RMSE value for the different tried hyperparameters. The best result is obtained with a neural network with two hidden layers with ?? neurons in the first layer and ?? in the second layer. In the next step the neural network is used for prediction on the test set.
...
```{r model nn rmse}
cat(" RMSE trainingset:  ", models$results$RMSE[length(models$results$RMSE)] * 
      (max(df_nn$price) - min(df_nn$price)) + min(df_nn$price), "\n",
    "Rsquared:          ", models$results$Rsquared[length(models$results$Rsquared)], "\n",
