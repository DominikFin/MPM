...
Models^[The aim of this study is to check the feasibility of the desired price-prediction feature, therefore it was decided to reduce the number of models to only five. Models were chosen according to their popularity in the respective country Germany. Every model shows a reasonable number of offers on the online platform.]: Audi A6 - Ford Fiesta - Opel Corsa - Seat Leon - Skoda Octavia

Attributes^[To ensure actuality of the data and as there was no count data in the original data set, it was decided to scrape actual data from the website.]: brand - model - currency - price - offer_type - power_ps - previous_owners (count data) - registration_year - registration_month - kilometer - fuel_type - gear_type - ZIP_code - crawl_timestamp
...
Continuous predictors: age_in_days - kilometer - power_in_ps - previous_owners^[For the application of a generalised linear model with family set to "Poisson" the variable "previous_owners" is used as response variable.]

Categorical predictors: car_model - offer_type - fuel_type - gear_type
...
```
The response variable price shows a right-skewed distribution. Furthermore, the price can be considered as "amount" Therefore it will be log-transformed in the modelling part. Age, power and kilometer could also be considered as right-skewed. By log-transforming these variables the predictions got worse in the modelling part. As the main goal of this feasibility study is to come up with accurate predictions it was omitted to log-transform the predictors.

Prices range from 390 to 77'950 Euros. 93% of all observations show a price which is 30'000 or less. Half of the cars offered are younger than four years (1460 / 365 ~ 4 years). Only 10% off all cars do have more than 200 PS and only 25 % of all cars do have a mileage which is above 100'000 kilometers.
```{r generalanalyis2}
...
create_barplot <- function(data, title) {
  p <- barplot(table(data), main = title, xpd = FALSE)
  text(x = p, y = table(data), label = table(data), pos = 1, cex = 0.8)
}

create_barplot(df$car_model, "Car models")
create_barplot(df$gear_type, "Gear types")
create_barplot(df$fuel_type, "Fuel types")
create_barplot(df$offer_type, "Offer type")
...
It seems to be that cars with automatic gear type come with higher prices as well as cars with fuel type Diesel and ElektroBenzin are more expensive than cars with fuel type Benzin. This makes perfectly sense from a business point of view.

Overall, Jahreswagen have generally a slightly higher price than Gebrauchtwagen.

## Checking for different effects among the car models
...
# Interpreting the kilometer coefficient omitting the interactions term
1 - exp(10000*-3.249e-06)
```
...
Example of interpreting the kilometer coefficient omitting the interaction terms:
It is important to note that the interpretation of the coefficients is multiplicative as we applied the log-transformation on the response variable. The kilometer variable has a negative coefficient of -3.249e-06. Considering a car which has a mileage of 10'000 kilometers, we can multiply this coefficient with 10'000 and apply the exp function to it. This results in a coefficient of 0.9680321, which in turn means that the car loses about 3.2% of its value predicted by our model.
...
As we want a model that does not overfit and generalize well, i.e. one that performs well on new data, we randomly split up our data into a test and training set. Afterwards, we apply the linear model on the training data set and test it with the test data set.
...
```{r}
svm.train <- df %>% slice(indices)
svm.test_in <- df %>% slice(-indices) %>% select(-car_model)
svm.test_truth <- df %>% slice(-indices) %>% pull(car_model)
```
...
```{r}
confmatrix.svm_1<-confusionMatrix(svm.test_pred_1, svm.test_truth)
confmatrix.svm_1$table
confmatrix.svm_1$overall['Accuracy']
```
...
##### Evaluate the results (confusion matrix)
```{r}
confmatrix.svm_2<-confusionMatrix(svm.test_pred_2, svm.test_truth)
confmatrix.svm_2$table
confmatrix.svm_2$overall['Accuracy']
```
The resulting SVM of the second model reaches a prediction accuracy of around 70%. This a noticeable better accuracy compared to model 1 (linear kernel) and the multiple regression model, which both reached around 60%. The disadvantage of the second model is the higher complexity due to the radial kernel.

## Neural Network
In this section we try again to predict the price of a car but this time using a neural network. For this we prepare the dataframe by hot encoding all the categorical variables. Additionaly the cars with the fueltype "Elektrobenzin" are excluded because of the small amount of cars with this feature. 
```{r prepare df for NN}
# filter out fueltype Elektrobenzin
df_nn <- df[!(df$fuel_type=="ElektroBenzin"),]
# Hot encoding of categorical variables
dmy <- dummyVars("~ .", data = df_nn, fullRank = T)
df_nn <- data.frame(predict(dmy, newdata = df_nn))
df_nn <- select(df_nn, -fuel_type.ElektroBenzin)
```

The data is split in a training set used for training the neural network and a test set for an unbiased evaluation of the final model with unseen data during the training phase. Furthermore the numeric data is scaled with a simple min max scaling procedure. This is done for a potentially faster learning and faster convergence during training and because of the difference in magnitude between the numeric values.
```{r scale and split in training and test , results='hide', message=FALSE}
# split in training and test set with equal distribution of car models
set.seed(123)
df_nn <- df_nn %>%
  mutate(n= row_number()) %>%
  select(n, everything())
df_nn_train <- df_nn %>%
  group_by(car_model.Ford.Fiesta, car_model.Opel.Corsa, 
           car_model.Seat.Leon, car_model.Skoda.Octavia) %>%
  sample_frac(.8)
df_nn_test <- anti_join(df_nn, df_nn_train)
# max and min value for scaling
max <- apply(select(df_nn, -n), 2, max)
min <- apply(select(df_nn, -n), 2, min)
# scale both the training and the test set
df_nn_train_scaled <-as.data.frame(scale(select(df_nn_train, -n), 
                                         center = min, scale = max - min))
df_nn_test_scaled <- as.data.frame(scale(select(df_nn_test, -n), 
                                         center = min, scale = max - min))
```
The cadet framework is used for training the network and hyperparameter optimization. The RMSE was used as the metric to minimze. To make sure that the resulting RMSE is not just resulting by chance due to the random split in training and test set k-Fold cross validation is used. The separate test set is afterwards used to test the model on unseen data.
```{r model nn train, cache=TRUE, results='hide'}
set.seed(123)
tuGrid <- expand.grid(.layer1=c(5:12), .layer2=c(0,6,12), .layer3=c(0))
trCtrl <- trainControl(
  method = 'repeatedcv',
  number = 3,
  repeats = 2,
...
models <- train(
  x = df_nn_train_scaled %>% select(-price),
  y = df_nn_train_scaled %>% pull(price),
...
The above plot shows the resulting RMSE value for the different tried hyperparameters. The best result is obtained with a neural network with two hidden layers with 11 neurons in the first layer and 6 in the second layer. In the next step the neural network is used for prediction on the test set.
```{r model nn predict, out.height= "35%", fig.align='center'}
# predict price on test data
predict_nn <- predict(models, newdata = df_nn_test_scaled %>% select(-price))
# undo scaling for predicted prices
predict_nn_unscaled <- predict_nn * (max(df_nn$price) - min(df_nn$price)) + min(df_nn$price)
# calculate residuals 
residuals_nn <- df_nn_test$price - predict_nn_unscaled
# plot residuals and predicted values
plot(predict_nn_unscaled, residuals_nn, abline(0,0), ylab = "residuals", 
     xlab = "predicted prices for test set")
```
The plot above shows the residuals for the predicted prices of the test set. They fluctuate around the mean of 0 but it appears that the deviation increases with higher car prices. This could indicate that especially for higher priced cars things like optional equipment or the condition of the car have a bigger influence and therefore cause more deviation. This could be an option for a potential improvement of the model.
```{r model nn rmse}
cat(" RMSE trainingset:  ", min(models$results$RMSE) * 
      (max(df_nn$price) - min(df_nn$price)) + min(df_nn$price), "\n",
    "Rsquared:          ", models$results$Rsquared[which.min(models$results$RMSE)], "\n",
    "RMSE testset:      ", sqrt(mean((df_nn_test$price - predict_nn_unscaled)^2)))
```
The model explains around 91% of the variability in the training data set. The RMSE of the data used for training seems to be higher than the obtained RMSE on the test set with unseen data. This is usually not the case but a possible explanation could be, that the share of higher priced cars with a general higher error was lower in the test set than in the training set. The neural network is therefore off by around 3000 Euros on average and achieves similar results in comparison with the linear model.
...
Possible options that should speed up the selling of a car by increasing the visibility on the platform are "Super" for a price of 45 euros and "Turbo" for 75 euros. 
...
## Implementation
The previously defined optimization task is implemented in R with the following function:
```{r optim_function}
sell.time.optimization <- function(r, v_max){
  # include client specific parameters
  var1 <- 60 / r
  var2 <- 30 + v_max
...
```{r optim_1}
sell.time.optimization(r = 10000, v_max = 20)
```
...
```{r optim_2}
sell.time.optimization(r = 2000, v_max = 60)
```
...
# Conclusion
In the eyes of the project team, the feasibility study that was conducted resulted in a positive proof-of-concept. Both parametric and non-parametric models could be used to implement a price prediction function in online car markets. A nice add-on supporting customers and dealers would be the one shown in the optimization process. The additional feature allows for differentiation among platform providers.

A classification procedure offers the possibility to determine which categorical variables should be included in the model. It has been found that classical classification methods such as SVM are suitable for making meaningful, if not always correct, classifications.

For the final model deployment, the project team would choose a linear model. First, the linear model is easier to interpret and second, the non-parametric models showed only slightly more accuracy but comes with more training expense. 

However, the linear model has some limitations. The feasibility study above focused on models that had a large enough sample size. In particular, when there are interaction terms and factor variables with many levels, one must ensure that there are enough observations for all individual levels of the factor variables.

Further analysis:
A possible extension of this feasibility study would be to evaluate the various errors generated by the models (residual analysis) to further improve them. In addition, the final model should be subjected to cross-validation to ensure reliable evaluation. Due to space constraints, both were omitted from this study. Finally, the resulting model should be compared to industry standards to ensure proper evaluation.
